...
· line：把这个月压成一句话，四十字以内。写往后还用得着的——
  找了谁、办成了什么、多少钱、留了什么话、欠着什么。
  「这个月过得很辛苦」没用；「交了两块押金给老周，排上了扛包的班」有用。
...
function runMonthLocal(s, list) {
  ...
  return {
    delta: {
      ...
      memo: { line: `${hits.length ? '想做的事那时候还没有，白忙半个月；' : ''}${way.way || '打零工'}，净${gain >= 0 ? '进' : '出'} ${E.money(Math.abs(gain), E.currencyOf(s.year, s.month))}` },
