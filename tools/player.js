...
function careful(s, c, r) {
  ...
  if (d === 1) {
    return `这个月头一件事：把「${w.way}」这条路彻底问明白。找到干这行的人，` +
      `问清楚三件事——入行要什么（本钱、家伙、门路、谁的关系）、一个月能挣多少、什么时候结账。` +
      `问到具体的人名和地方，别问完就走。手里有 ${cash}，该花的打点钱就花。` +
      `问明白就当月开工，不等下个月。`;
  }
  if (d === 2) {
    return `按上个月问到的，这个月把入行要的东西凑齐：该交的押金交了，该买的家伙买了，` +
      `该求的人这个月就去求。凑不齐就退而求其次，找个不要门槛的先做着，` +
      `${alt.length ? `比如「${alt[0].way}」` : '什么都行'}。**这个月必须开工，不许再打听了。**`;
  }

  let t = `接着做「${w.way}」，这个月从月初干到月底，不再东问西问。` +
    `按谈好的价钱和工时做，能多接一份就多接一份。`;

  if (d % 6 === 0) {
    t += `月底算一次总账：这条路一个月到底净剩多少，够不够本。` +
      `不够就下个月换「${alt.length ? pick(r, alt).way : '别的营生'}」，够就想办法把量做大。`;
  } else if (d % 3 === 0) {
    t += `把手里的 ${cash} 用上：能雇个人搭把手就雇，能多进一批货就进，` +
...
function lazy(s) {
  return ['随便走走。', '这个月不想动。', '睡一个月。', '看看天。', '待着。'][s.n % 5];
}
