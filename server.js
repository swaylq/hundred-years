...
function view(s) {
  ...
  return {
    year: s.year, month: s.month, day: Math.min(s.day, E.DAYS), days: E.DAYS,
    finished: s.day > E.DAYS,
...
function dayList(s) {
  return (s.days || []).map(d => ({
    day: d.day, list: d.list, story: d.story, tally: d.tally,
...
const routes = {
  ...
  'POST /api/day': async (req, res) => {
    ...
    if (busy.has(lockKey)) return oops(res, 409, '这一天正在算，等它出来再点');
    busy.add(lockKey);
    try {
  ...
  'POST /api/settle': async (req, res) => {
    ...
    const s = found.state;
    if (s.status === 'done') return json(res, 200, { result: JSON.parse(found.row.result), already: true, days: dayList(s) });
    if (s.days.length < E.DAYS) return oops(res, 400, `还差 ${E.DAYS - s.days.length} 天`);
    if (busy.has(String(b.id))) return oops(res, 409, '还有一天在算，等它出来再结');
    E.advanceTo(s, E.DAYS);
    const r = E.settle(s);
  ...
  'GET /api/story': async (req, res, q) => {
    ...
    if (!found) return oops(res, 404, '找不到这一局');
    json(res, 200, { days: dayList(found.state), nick: found.state.nick, year: found.state.year, month: found.state.month });
  },
...
/* 真正算一天的那一段，从路由里拆出来，好让上面那把锁包住它 */
async function runOneDay(req, res, b, s) {
    ...
    if (HAS_KEY) {
      ...
      if (!r.ok) {
        ...
        out = SIM.runDayLocal(s, b.list); usedLocal = true;
        out.why = r.why === 'ip' ? '你这十分钟问得太密了，这一天先由本地算' : '这会儿全站都很忙，这一天先由本地算';
      } else {
        try { out = await SIM.runDay(s, b.list); }
        catch (err) { out = SIM.runDayLocal(s, b.list); usedLocal = true; out.why = String(err.message).slice(0, 120); }
      }
    } else { out = SIM.runDayLocal(s, b.list); usedLocal = true; }

    const curNow = E.currencyAt(s.year, s.month, s.day);
    ...
    if (s.day > E.DAYS) s.day = E.DAYS + 1;
    DB.saveRun(b.id, s);
